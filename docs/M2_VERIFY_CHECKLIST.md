...
| A5 | 三目录落点 | 看 hilog 第⑦桩 | `documents: 系统目录可写`(非 `降级 → el2/Documents`) |
...
| # | 项 | 操作 | 期望 |
|---|---|---|---|
| B1 | 关联注册 | 文件管理器长按 .docx → 打开方式 | 列表含 GenOffice |
| B2 | argv 链路 | 双击 .docx 启动,看 hilog | `open-document: file://... -> /storage/...` |
| B3 | 文档真打开 | 同上,看 UI | 进 docs 模块并渲染该文档(**空内容 = 路径/权限问题**) |
| B4 | 六类抽验 | xlsx / pptx / pdf / md / html | 各进对应模块 |
| B5 | 未知类型 | 双击 .txt | 回落 Home(不崩) |
