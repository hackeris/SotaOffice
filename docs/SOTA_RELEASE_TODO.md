...
**做法见 `BRAND_REPLACEMENT.md`**（分类原则、名称映射、脚本设计、批次与验证）。本节只列要动的位置。

**规模**：`Genspark` 系列（三种写法）约 2850 处 / 250 文件，`GenOffice` 约 1600 处 / 176 文件。
另有 `GENOFFICE_*` 环境变量与 `@genoffice/*` 包名约 1700 处——那是内部标识，**不动**。

| app | 文件数 | genspark 提及次数 |
| --- | --- | --- |
| shell | 14 | 494 |
| docs | 73 | 323 |
| slides | 70 | 267 |
...
**注意这里大半是死文案**：账号链移除后，描述登录、积分、账号的 key 已经没有对应 UI 了。
它们要**删**，不是要换品牌——换成新品牌等于给不存在的功能做广告。
