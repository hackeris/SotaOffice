#!/bin/bash
# rebuild-drill.sh —— 毁灭性重建演练(构建可复现性证明)
#
# 正确命令:bash scripts/rebuild-drill.sh
...
# 必须含 Native 子进程链的编译产物:xlsx-engine 的 cargo target 与 xlsx-launcher
# 的 .so——删掉它们后 build-genoffice 的存在性守卫会触发真正的从零交叉编译,
# 演练才对新链(交叉编译 + 启动壳 build.sh)有背书;只删 entry/libs 的话,
# cargo 旧产物原样拷回,演练对编译链是空转。
...
build-profile.json5 \
thirdparty/genoffice/apps/sheets/native/xlsx-engine/target \
thirdparty/genoffice/apps/sheets/native/xlsx-launcher/libxlsx_launcher.so"
...
echo "==> [4/6] build-genoffice --no-build(产物组装 + Native 子进程链从零交叉编译 + 断言)"
