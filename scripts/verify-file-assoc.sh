...
# 前提:    ① HAP 已装机,且**冷启动过一次**——桌面探针文件由 shim 桩⑱ 在应用侧生成
...
# 用法:    bash scripts/verify-file-assoc.sh 192.168.1.5:44959
#
# 背景:文件关联需要公共目录里的真实文件,而 hdc 侧一律写不进去(2026-09-24 实测):
#      `ls /storage/Users` 在 shell 命名空间报 No such file or directory,
#      `hdc file send` 到同路径报 "Error opening file"。故测试文件由 shim 桩⑱
#      (scripts/shim/main-shim.mjs)在应用进程内生成。
set -eo pipefail
